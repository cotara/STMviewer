...
firFilter::firFilter()
{
    ...
    while (!in.atEnd()) {
        ...
        koefs[k]=line.toDouble()/pow(2,32);
        k++;
    }
    offset=k;//201
}
...
QByteArray firFilter::toFilter(QByteArray &in,int len, QString filename){
...
QVector <QVector<double>> firFilter::extrFind(QByteArray &in,int len){
    ...
    for(int j=1;j<len;j++){
       ...
       if(ch>chPrev){                                               //График пошел вверх
            ...
            if(downVal>6){                                         //Нашли значительный минимум
                x.append(j-1-straightCount/2);
                maxmin.append(chPrev);
            }
            downVal=0;
            straightCount=0;
