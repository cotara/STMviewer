...
void AutoFindWizard::init(QVector<double> params)
{
    ...
    if(params.size() == 6){
        ui->NxLabel->setText(QString::number(params.at(0),'g',10));
        ui->NyLabel->setText(QString::number(params.at(1),'g',10));
        ui->HxLabel->setText(QString::number(params.at(2),'g',10));
        ui->HyLabel->setText(QString::number(params.at(3),'g',10));
        ui->CxLabel->setText(QString::number(params.at(4),'g',10));
        ui->CyLabel->setText(QString::number(params.at(5),'g',10));
    }
    else{
        ui->NxLabel->clear();
        ui->NyLabel->clear();
...
//Для теста:
    ...
    QVector <double> temp;
    //Данные из эксельки для диаметра 6.0
...
//Переопределенный Cancel в визарде
...
void AutoFindWizard::autoFindAlg()
{
    ...
    initNx = m_params.at(0);
    initNy = m_params.at(1);
    initHx = m_params.at(2);
    initHy = m_params.at(3);
    initCx = m_params.at(4);
    initCy = m_params.at(5);
...
//#pragma omp parallel  private(err,ila,iNx,iNy, iHx,iHy,iCx,iCy)
   //{
    #pragma omp parallel for private(err,iNx,iNy, iHx,iHy,iCx,iCy) //nowait//ordered //schedule(static)
                ...
                for ( iHx = 0;iHx<(ui->HxRange->value())*2+1;iHx++){
                    for ( iNy = 0;iNy<(ui->NyRange->value())*2+1;iNy++){
                        for ( iNx = 0;iNx<(ui->NxRange->value())*2+1;iNx++){
                            err = calcErrDiemeter(iNx,iNy,iHx,iHy,iCx,iCy);
                    ...
                    emit setProgress(currentStep*100/steps);
                }
            }
        }
    }
  //}
    ...
    on_startFind_clicked(false);//Отжимаем кнопку автоподбора
    ...
    for(int i=0;i<9;i++){
        QVector<double> temp = calcDiemeter(allFronts.at(i),0,0,0,0,0,0);
        qDebug()<< "Rx="<<temp.at(0) << "   Ry="<<temp.at(1);
    }
}

//Расчет радиусов по пачке экстремумов и параметров
QVector<double> AutoFindWizard::calcDiemeter(QVector<double> dots, int iNx,int iNy,int iHx,int iHy,int iCx,int iCy){
    QVector<double> x;
    double Nx = NxV[iNx];
    double Ny = NyV[iNy];
    double Hx = HxV[iHx];
...
{
    QVector<double> par {bestNx,bestNy,bestHx,bestHy,bestCx,bestCy};
